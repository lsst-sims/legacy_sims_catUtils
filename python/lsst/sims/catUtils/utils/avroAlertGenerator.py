...
class AvroAlertGenerator(object):
    ...
    def _create_objects(self, diaobject_data):
        ...
        for i_object in range(len(diaobject_data)):
            ...
            avro_diaobject['pmRa'] = 0.0
            avro_diaobject['pmDecl'] = 0.0
            avro_diaobject['parallax'] = 0.0
            pm_parallax_cov = {}
    ...
    def write_alerts(self, obshistid, data_dir, prefix_list, htmid_list, n_obshistid,
        ...
        with DataFileWriter(open("%s_%d.avro" % (out_file_root, obshistid), "wb"),
            ...
            diaobject_query = 'SELECT alert.uniqueId, alert.ra, alert.dec, meta.TAI '
            diaobject_query += 'FROM alert_data as alert '
            diaobject_query += 'INNER JOIN metadata as meta ON meta.obshistId=alert.obshistId '
            diaobject_query += 'GROUP BY alert.uniqueId'
