...
class _baseAlertCatalog(PhotometryBase, CameraCoordsLSST, _baseLightCurveCatalog):
    ...
    def get_alertFlux(self):
        ...
        snr_diff = np.abs(dflux)/diff_sigma
        if len(diff_sigma)>0:
...
class AlertDataGenerator(object):
    ...
    def _output_alert_data(self, conn, data_cache, dmag_cutoff):
        ...
        for i_cache_tag, cache_tag in enumerate(data_cache):
            ...
            xPix_val = np.round(active_cache['xPix']/pix_unit).astype(int)
        ...
        with sqlite3.connect(db_name, isolation_level='EXCLUSIVE') as conn:
            creation_cmd = '''CREATE TABLE alert_data
                           (localId int, obshistId int,
                            xPix_val int, yPix_val int,
                            chipNum int,
                            dflux_unit int, dflux_val int, dflux_sign int,
                            flux_sigma_unit int, flux_sigma_val int,
            ...
            cursor.execute('CREATE INDEX unq_obs_quiescent ON quiescent_obs (localId)')
            conn.commit()
