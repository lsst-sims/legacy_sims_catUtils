...
class ChunkIteratorTestCase(unittest.TestCase):
    ...
    def test_bulge_obj(self):
        ...
        colnames = ['raJ2000', 'decJ2000', 'galtileid', 'magNorm', 'majorAxis']

        fatboy_obj = GalaxyBulgeObj(database='LSSTCATSIM',
                                    host='fatboy.phys.washington.edu',
                                    port=1433,
                                    driver='mssql+pymssql')

        local_obj = LocalGalaxy.LocalGalaxyBulgeObj(database='LSSTCATSIM',
                                                    host='fatboy.phys.washington.edu',
                                                    port=1433,
                                                    driver='mssql+pymssql')
        ...
        t_fatboy = time.time()-t_start
        print('\nin Bulge: t_local %e t_fatboy %e' % (t_local, t_fatboy))
    ...
    def test_disk_obj(self):
        ...
        fatboy_obj = GalaxyDiskObj(database='LSSTCATSIM',
                                   host='fatboy.phys.washington.edu',
                                   port=1433,
                                   driver='mssql+pymssql')

        local_obj = LocalGalaxy.LocalGalaxyDiskObj(database='LSSTCATSIM',
                                                   host='fatboy.phys.washington.edu',
                                                   port=1433,
                                                   driver='mssql+pymssql')
        ...
        for chunk in fatboy_iter:
            for name in colnames:
                fatboy_results[name].append(chunk[name])
        t_fatboy = time.time()-t_start
        print('\n in Disk: t_local %e t_fatboy %e' % (t_local, t_fatboy))
    ...
    def test_agn_obj(self):
        ...
        for chunk in fatboy_iter:
            for name in colnames:
                fatboy_results[name].append(chunk[name])
        t_fatboy = time.time()-t_start
        print('\nin AGN: t_local %e t_fatboy %e' % (t_local, t_fatboy))
